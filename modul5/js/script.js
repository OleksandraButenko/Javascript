...
/*const summAllNumbers = (...args) => args.reduce(function (acc, el) {
    return acc + el;
   }, 0);*/
/*
   function summAllNumbers(...args){
       const reduce = args.reduce((acc,el) =>acc + el,0)
       return(reduce)
   }
...
const users = [
  ...
  const setGuestState =(users, period) => users.map(el => ({...el,isActive: el.inactiveDays < period}))
...
*/
 /*function getTotal(order){
     ...
     let b = a.reduce((acc, el) => acc + el, 0)
     return b;
 }*/
...
/*let a = scientist.reduce((acc,el)=> acc += el.passed - el.year,0 );
console.log(a)*/
...
/*let a = scientist.sort((a,b) => a.year > b.year? 1 : - 1);
console.log(a)*/
...
/*let a = scientist.filter( el => !(el.element >1400 && el.element < 1700))
console.log(a)*/
...
/*let a = scientist.find( el => el.first = 'Albert Einstein')
console.log(a.year)
*/

// 9) Знайти вчених прізвище яких починається на літеру С

/*let a = scientist.filter( el => el.last.charAt('C')=== 'C');
console.log(a)
*/

// 10) Видалити з масива всіх вчених імя яких починається на A
/*
let a = scientist.filter( el => el.first.charAt() !== 'A');
console.log(a)*/
 ...
 */
/*const getUsersByEyeColor = (arr, color) =>users.filter( el => el.eyeColor === color).map(el => el) ;
...
//  */
/*
const getInactiveUsers = (arr, isActive1) => users.filter( el => !el.isActive === isActive1).map(el => el) ;
...
//  * Получить пользоваля (не массив) по email (поле email, он уникальный)
 /*const getUserByEmail = (arr, email) => users.filter(el => el.mail === email)
