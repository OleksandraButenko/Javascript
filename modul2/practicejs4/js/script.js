...
let newMass = []
for(let i = 0; 1 < mass.length; i += 1){
    if(mass[i][0] === "M"){
        newMass.push(mass[i] || mass[i][0] === "M" ){
...
const mass = ['Maksim', 'Anna', 'Pasha', 'Vova'];
...
/*let counter = 3
do{
    let password = Number(prompt('Enter passsword'));
    counter -= 1
}while(counter)
*/
...
/*
let mass = ["Mango", "Poly", "Ajax", "Lux", "Jay", "Kong"];
let a = 0;
...
/*
  ...
  Пусть prompt говорит "Введите цифру между x y u", где x и y 
  соотвественно самый маленький и самый большой элемент массива.
...
/*
let numbers = [12, 15, 25, 37, 41, 62, 74, 83]
let min=12;
let max=83;
let input;
...
do{
    let input = Number(prompt(`Введите цифру межу ${min} и ${max}`));
    console.log(input);
